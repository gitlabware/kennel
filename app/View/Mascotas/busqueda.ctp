...
<?php endif; ?>
<div class="innerLR">
    <h3>RESULTADO BUSQUEDA DE MASCOTA</h3>
            ...
            <table class="dynamicTable tableTools table table-striped table-bordered table-condensed table-white">
                ...
                <tbody>
                    <?php foreach ($mascotas as $m): ?>
                      <?php
                      $clase = '';
                      if ($m['Mascota']['verificado'] == 1) {
                        $clase = 'style="background-color: #d9edf7;"';
                      }
                      ?>
                      <tr>
                          <td <?php echo $clase;?>><?php echo $m['Mascota']['kcb'] ?></td>
                          <td <?php echo $clase;?> id="fila<?php echo $m['Mascota']['id'] ?>">
                              <div id="cfila<?php echo $m['Mascota']['id'] ?>">
  <?php echo $m['Mascota']['nombre_completo']; ?>
                              </div>
                          </td>
                          <td <?php echo $clase;?>><?php echo $m['Mascota']['num_tatuaje'] ?></td>
                          <td <?php echo $clase;?>><?php echo $m['Raza']['nombre']; ?></td>
                          <td <?php echo $clase;?> class="center">
  <?php echo $m['Propietarioactual']['nombre'] ?>
                          </td>
                          <td <?php echo $clase;?> class="hidden-print">
                              <?php if ($this->Session->read('Auth.User.role') == 'administrador'): ?>
                                <a href="#myModal" data-toggle="modal" class="label label-info" onclick="$('#imgcargando').toggle();
                                    $('#mimodal').toggle();
                                    $('#mimodal').load('<?php echo $this->Html->url(array('controller' => 'Mascotas', 'action' => 'ajaxmascota', $m['Mascota']['id'])); ?>', function () {
