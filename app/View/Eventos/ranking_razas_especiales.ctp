        ...
        <table class="table table-bordered table-white">
            ...
            if(!empty($ano))
            {
                $ranking = $evenmaspun->find('all',array('recursive' => 2
                    ,'conditions' => array('EventosMascotasPuntaje.raza_id' => $ra['Raza']['id'],'Year(Evento.fecha_inicio)' => $ano,'EventosMascotasPuntaje.categoriaspista_id' => 1)
                ...
                $ranking = $evenmaspun->find('all',array('recursive' => 2
                    ,'conditions' => array('EventosMascotasPuntaje.raza_id' => $ra['Raza']['id'],'EventosMascotasPuntaje.evento_id' => $idEvento,'EventosMascotasPuntaje.categoriaspista_id' => 1)
                    , 'group' => array('EventosMascotasPuntaje.mascota_id')
                                , 'fields' => array('Mascota.nombre_completo', 'SUM(EventosMascotasPuntaje.puntos) puntos', 'Mascota.propietarioactual_id','EventosMascota.catalogo')
                                , 'order' => 'SUM(EventosMascotasPuntaje.puntos) DESC'
                    ));
